...
import { puneGroupForGroupNumber } from "@/lib/pune";
import { supabase } from "@/lib/supabaseClient";
import { LeaderboardRow } from "@/lib/types";

export default function PuneLeaderboardPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-8 sm:px-6">
        ...
        {rows && rows.length > 0 ? (
          <LeaderboardTable rows={rows} resolveGroup={puneGroupForGroupNumber} />
        ) : null}
      </main>
    </div>
  );
